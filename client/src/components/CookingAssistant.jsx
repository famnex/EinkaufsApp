...
export default function CookingAssistant(props) {
    const { isOpen, onClose, recipe, audioContext, hasActiveAlarm, onStatusChange } = props;
    ...
    // Notify parent about status changes for the toggle button
    useEffect(() => {
        if (onStatusChange) {
            onStatusChange({ isListening, isStandby });
        }
    }, [isListening, isStandby, onStatusChange]);
        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div
                    ...
                    className="fixed left-4 md:left-8 bottom-[calc(1.5rem+env(safe-area-inset-bottom))] z-[250] w-[90vw] md:w-96 h-[500px] max-h-[80vh] bg-card border border-border rounded-3xl shadow-2xl flex flex-col overflow-hidden transition-all duration-300"
                >
                    {/* Header */}
